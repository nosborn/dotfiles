...
function! s:GitBlameUpdateVirtualTextIfDifferentLine(buffer, line)
  if (a:buffer != s:prevBuffer || a:line != s:prevLine)
...
function! s:GitBlameData(buffer, line)
  ...
  let s:line = a:line
  let blameCommand = 'git blame -L' . a:line.','.a:line . ' -p ' . bufname(a:buffer)
...
function! s:GitBlameSetVirtualText(id, data, event)
  let s:jobId = 0
  if (line('.') == s:line)
    call nvim_buf_set_virtual_text(s:buffer, s:gitBlameNsId, s:line - 1, [[s:GitBlameComposeText(a:data), 'Comment']], [])
  endif
endfunction
